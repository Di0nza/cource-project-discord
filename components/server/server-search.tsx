...
export const ServerSearch = ({
    ...
    const onClick = ({ _id, type }: { _id: string, type: "channel" | "member"}) => {
        setOpen(false);

        if (type === "member") {
            return router.push(`/servers/${params?.serverId}/conversations/${_id}`)
        }

        if (type === "channel") {
            return router.push(`/servers/${params?.serverId}/channels/${_id}`)
        }
    }
                    ...
                    {data.map(({ label, type, data }) => {
                        ...
                        return (
                            <CommandGroup key={label} heading={label}>
                                {data?.map(({ _id, icon, name }) => {
                                    return (
                                        <CommandItem key={_id} onSelect={() => onClick({ _id, type })}>
                                            {icon}
                                            <span>{name}</span>
                                        </CommandItem>
                                    )
